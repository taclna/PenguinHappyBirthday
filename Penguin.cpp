...
Penguin::Penguin(SDL_Renderer* renderer, const std::vector<std::string>& candleOffImgs,
    ...
    : currentState(CANDLE_OFF), animationFrame(0), lastFrameTime(0), frameDuration(100) {
...
void Penguin::setVolume(float volume) {
    ...
    std::cout << volume << '\n';
    if (currentState == CANDLE_ON && currentVolume > 50.0f) {
